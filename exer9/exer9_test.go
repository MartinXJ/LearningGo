...
// Added more testing to test point scale
...
// Added more to test point Rotate
func TestPointRotate2(t *testing.T) {
	p := Point{2, 1}
	// compare as float32 to avoid rounding differences
	p.Rotate(math.Pi / 2)
	assert.Equal(t, float32(-1.0), float32(p.x))
	assert.Equal(t, float32(2.0), float32(p.y))
...
func TestSortingSmall(t *testing.T) {
	...
	if !reflect.DeepEqual(arr, []float64{1, 3.1, 4, 5, 7, 7.2, 9.1}) {
		t.Error("Arr ay not actually sorted", arr)
	}
}
...
// change the length to 10,000 (just to test)
